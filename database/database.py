        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    role VARCHAR(20) NOT NULL CHECK (role IN ('resident', 'security', 'admin')),
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_passes_user_id ON passes(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_passes_status ON passes(status)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_passes_car_number ON passes(car_number)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_passes_created_at ON passes(created_at)")
    ...
    async def create_user(self, user: User) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO users (telegram_id, role, full_name, phone_number, apartment, status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                user.telegram_id, user.role, user.full_name, user.phone_number,
                user.apartment, user.status, datetime.now(), datetime.now()
            ))
            await db.commit()
